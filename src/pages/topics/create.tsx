...
import { TOPIC_CATEGORY } from "@/constants/category.contant";
import { ArrowLeft, Asterisk, BookOpenCheck, ImageOff, Save } from "lucide-react";

export default function CreateTopic() {
    return (
        <main className="m-full h-full min-h-[1024px] flex gap-6 P-6">
            <div className="fixed right-1/2 bottom-10 translate-x-1/2 z-20 flex items-center gap-2">
                <Button variant={"outline"} size={"icon"}>
                    <ArrowLeft  />
                </Button>
                <Button variant={"outline"} size={"icon"} className="w-22 !bg-yellow-800/50">
                    <Save  />저장
                </Button>
                <Button variant={"outline"} size={"icon"} className="w-22 !bg-emerald-800/50">
                    <BookOpenCheck  />발행
                </Button>
            </div>  
